...
Deploying via AWS Serverless assumes that you have setup the [credentials](https://www.serverless.com/framework/docs/providers/aws/guide/credentials/) for the Serverless Framework on your computer.

You'll have to add the serverless packages to your api: `yarn workspace api add serverless`

To setup your Redwood project to use AWS Serverless run: `yarn rw setup deploy aws-serverless`

Once that's complete you can invoke a deployment via: `yarn rw deploy aws`.

This command will take care of building, packaging, and shipping your AWS Serverless functions.
